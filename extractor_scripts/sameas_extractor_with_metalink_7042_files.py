...
log_file = open( which + "_laundromat_metalink_Oct18.nt.log", 'w')
...
file_no_metalink  = open( which + "_without_metalink_Oct18.tsv", 'w')
...
for f, _, _ in triples:
	file_md5_list.append(f)

print ('there are in total ', len(file_md5_list), ' files that has been identified to have sameAs triples')

map_dir_to_file = {}
for f in file_md5_list:
	name = f[43:]
	d = name[:2]

	if d not in map_dir_to_file.keys():
		map_dir_to_file[d] = [name]
	else:
		map_dir_to_file[d].append(name)

	# print (f, ' with ', name, ' in_dir ', d)

with open( which + "_laundromat_metalink_Oct18.nt", 'w') as output:
	...
	for t in top_dir:
		...
		# print ('the original filelist = ', filelist)
		# total_files_processed = 0

		filelist = []
		if t in map_dir_to_file.keys():
			print ('the focuesed list in this dir', map_dir_to_file[t])
			for f in map_dir_to_file[t]:
				filelist.append(ZIPFILES_PATH + t + '/' + f + '/data.nq.gz')
			# print ('the focused filelist = ', filelist)
		...
		for gzfile in filelist: # may skip the first 1000 , there is no decoding error
			...
			count_processed = 0
			count_processed_sameas = 0

			while True: # if not found after 1000, jump to next file
				bline = ''
				line = ''
				last = ''
				try:
					...
					with open(which+"_exception_oct18.txt", "a") as error:
			...
			log_file_writer.writerow([gzfile, count_sameAs_statement, count_sameAs_statement_with_metalinkID, time_formated])
			log_file.flush()
